...
const MapComponent = ({ mainLocation, locations }) => {
  ...
  return (
    <GoogleMap
      ...
      zoom={10}
      center={{
        lat: mainLocation?.lat || center.lat,
        lng: mainLocation?.lng || center.lng,
      }}
    >
      {mainLocation && (
        <Marker
...
const ShowTaemem = () => {
  ...
  useEffect(() => {
    const fetchTaemem = async () => {
      ...
      try {
        const response = await axios.get(`${API_BASE_URL}/api/v1/taemems/${id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
            Accept: 'application/json',
            password: '$2y$12$lKLPBP1GlcywPnqPZceE4OcTWQNMrTgoshgoz91DrvvuTFMGiUI32',
        ...
        console.log('Taemem Data:', response.data.data);
    ...
    <div className="container mt-3">
          ...
          <div className="d-flex">
            <div className="col-right">
              ...
              <div className="list-group mx-2">
                <h5>
                  <strong>عنوان التعميم:</strong>
                  <span className="text-secondary mx-2">
                    {taememData?.title || 'تفاصيل التعميم'}
                  </span>
                </h5>
                <h5>
                  <strong>مفقود منذ:</strong>
                  <span className="text-secondary mx-2">
                    {taememData?.lost_date || 'غير متوفر'}
                  </span>
                </h5>
                <h5>
                  <strong>آخر مرة شوهد فيها:</strong>
                  <span className="text-secondary mx-2">
                    {taememData?.locations[0]?.date || 'غير متوفر'}
                  </span>
                </h5>
        ...
        <div className="card-2 section-card">
          ...
          <div className="card-body">
            <p className="text-secondary mx-2">
              {taememData?.description || 'لا توجد ملاحظات'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="map-contain d-flex mt-5">
            ...
            {taememData?.locations?.map((location) => (
              <li key={location.id}>
                ...
                <div className="icons d-block mx-3" style={{ float: 'left' }}>
                  ...
                  {taememData.locations.map((location) => (
                    <div key={location.id} className="location-item"></div>
                  ))}
                  {loggedInUserId === taememData?.user?.id && (
                    <FontAwesomeIcon
                      icon={faTrash}
        ...
        <div className="map w-50">
          <MapComponent
            mainLocation={taememData?.main_location}
            locations={taememData?.locations}
          />
        </div>
      </div>
